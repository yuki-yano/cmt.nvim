...
local function denops_request(method, payload)
  if not vim.fn.exists('*denops#request') then
    return false, { status = 'fallback', payload = { mode = 'line' } }
  end
  local args = {}
  if payload ~= nil then
    args = { payload }
  end
  local ok, result = pcall(vim.fn['denops#request'], 'cmt', method, args)
  if not ok then
    return false, { status = 'fallback', payload = { mode = 'line', error = result } }
  end
  return true, result
end
...
local function dispatch(payload)
  ...
  local ok, result = denops_request('toggle', payload)
  if not ok then
    log('error', 'cmt.nvim: toggle request failed' .. format_reason(result.payload))
    fallback_gc(payload.scope)
    return ''
  end
...
function Ops.open(direction)
  ...
  local ok, result = denops_request('openComment', {
    direction = direction,
  })
  if not ok then
    fallback_gc('line')
...
function Ops.info()
  if not vim.fn.exists('*denops#request') then
    vim.notify('cmt.nvim: Denops is not available', vim.log.levels.ERROR, { title = 'cmt.nvim' })
    return
  end
  local ok, result = denops_request('info', {})
  if not ok then
    log('error', 'cmt.nvim: failed to fetch info' .. format_reason(result.payload))
    return
  end
  if result.status ~= 'ok' then
